...
const Bookings = () => {
  const { bookings, verifyBookingOtp, setBookingOtp } = useStore()
  ...
  // Subscribe to start OTP broadcasts so host sees the same code generated by customer
  useEffect(() => {
    const channels = currentBookings
      ...
      .map((b) => {
        ...
        ch.on('broadcast', { event: 'otp_start_issued' }, (payload) => {
          ...
          if (code) {
            setBookingOtp(b.id, 'start', String(code))
            setOtpInputs(prev => ({ ...prev, [b.id]: String(code) }))
          }
        })
        ch.subscribe()
        return ch
